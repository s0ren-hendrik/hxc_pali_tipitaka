...
- isIE6Client()				Detect if client browser is IE6
- mobile_device_detect()		Detect if client is mobile or desktop
- utf8tohtml()				UTF8 string to HTML
- _substr()					Substring without losing word


*/

function title2link($input)
{
...
function cutx($in,$nbchars=60)
{
	$in = str_replace("<br/>"," ",$in);
	$in=strip_tags($in);
	
	if(strlen($in)>$nbchars)
	{
		$out = substr($in,0,$nbchars)."...";
	}else
	{
		$out = $in;
	}
...
function isIE6Client($userAgent)
{
	if(preg_match("/MSIE 6/i", $userAgent))
	{
		return true;
	}else
	{
...
function utf8tohtml($utf8, $encodeTags)
{
    ...
    for ($i = 0; $i < strlen($utf8); $i++) {
        ...
        if ($ascii < 128) {
            // one-byte character
            $result .= ($encodeTags) ? htmlspecialchars($char) : $char;
        } else if ($ascii < 192) {
            // non-utf8 character or not a start byte
        } else if ($ascii < 224) {
            // two-byte character
            $result .= htmlspecialchars(substr($utf8, $i, 2), ENT_QUOTES, 'UTF-8');
            $i++;
        } else if ($ascii < 240) {
